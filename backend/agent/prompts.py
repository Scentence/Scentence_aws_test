...
[분류 기준]
1. **interviewer (추천 요청)**: 
   - 사용자가 자신이나 타인을 위해 향수를 **추천**해달라고 요청하는 경우.
   - 정보가 충분하든 부족하든 상관없음. "추천"이라는 의도가 보이면 무조건 이쪽으로 분류.
   - 예: "남자 향수 추천해줘", "여름에 뿌릴만한 거 있어?", "여자친구 선물 뭐 사지?"

2. **info_retrieval (지식/정보 검색)**:
   - 특정 향수, 브랜드, 노트(원료), 어코드(향조)에 대한 **지식이나 정보**를 묻는 경우.
   - 예: "샤넬 넘버5 어때?", "베티버가 무슨 향이야?", "시트러스랑 우디 차이가 뭐야?", "딥티크 브랜드 설명해줘"
...
[주의사항]
- "상큼한 향수 추천해줘" -> `interviewer` (추천 의도)
- "상큼한 향수가 뭐야?" -> `info_retrieval` (지식 질문)
- 모호한 경우, 사용자가 **'무엇을 살지(Decision)'** 고민 중이면 `interviewer`, **'무엇인지(Fact)'** 궁금해하면 `info_retrieval`로 분류하세요.
"""
...
1. **정보 부족 시**:
   - Target이 없으면 대상 질문.
   - Target은 있는데 Concept이 없으면 "선호하는 분위기" 하나만 질문.
   - 질문은 한 번에 하나씩만 하세요.
2. **정보 충족 시**:
   - 이미 A(대상)와 B(컨셉)가 확보되었다면, **절대로** 추가 질문을 하지 마세요.
   - 즉시 `is_sufficient: true`를 반환하여 Researcher가 실행되도록 하세요.
"""
